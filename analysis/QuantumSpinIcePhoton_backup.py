...
def main_dispersion():
    # Set up parameters for the quantum spin ice photon Hamiltonian
    Jzz = 1
    Jpm = 0.04
    # Calculate g0 and Kappa
    g0 = 24 * np.abs(Jpm**3)/Jzz**2
    Kappa = g0  # Following Benton's convention
    speed_of_light = (0.51 * Kappa)/2
    temperature = 0.1
    ...
    ham = QuantumSpinIcePhoton(Jzz=Jzz, Jpm=Jpm, renorm_param=1, kappa=1, W=0, temperature=temperature*speed_of_light)

    # Define high symmetry points path: Gamma -> X -> W -> K -> L -> Gamma
    points = [ham.FBZ_Gamma_GCC, ham.FBZ_X_GCC, ham.FBZ_W_GCC, ham.FBZ_K_GCC, ham.FBZ_L_GCC, ham.FBZ_Gamma_GCC]
    n = 100

    # Plot dispersion
    print("Plotting dispersion along high symmetry points...")
    plot_dispersion_along_point(ham, points, n=n, savefig=True, file_output_name="dispersion_high_symmetry.pdf")
    print("Dispersion plot saved to 'dispersion_high_symmetry.pdf'")

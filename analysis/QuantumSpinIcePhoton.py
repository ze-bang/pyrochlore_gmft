...
class QuantumSpinIcePhoton(object):
    ...
    def __init__(self, speed_of_light=None, W=0, kappa=1, temperature=0.0, renorm_param=1, Jpm=0, Jzz=1, B=0, field_dir=np.array([0,0,1])):
        ...
        self.g0 = 12 * np.abs(self.Jpm**3)/self.Jzz**2 + 5 * self.Jpm**2/np.abs(self.Jzz**3) * B**2 * np.dot(self.z_local_gcc_vec, field_dir)**2
        ...
        # Compute speed_of_light if not provided
        if speed_of_light is None:
            self.speed_of_light = (self.c_moessner * tildeKappa * renorm_param) / 2
        else:
            self.speed_of_light = speed_of_light

        # self.U = self.speed_of_light**2/(tildeKappa * renorm_param) # Following Benton's convention
        self.U = 0.0004266063436816387
        self.W = W
        self.kappa = kappa
    ...
    def plot_dispersion_along_path(self, path_labels=None, npoints=50, field_dir=None, savefile=None):
        """Compute and plot dispersion along a high-symmetry path.

        Parameters
        ----------
        path_labels : list of str or None
            Sequence of high-symmetry point keys (e.g. ['Γ','X','W','K','Γ'])
            If None, defaults to ['Γ','X','W','K','Γ','L','U','W','L','K'].
        npoints : int
            Number of interpolation points between successive high-symmetry points.
        field_dir : array-like or None
            Magnetic field direction used to compute Kappa. If None, uses the
            instance field_dir if present, otherwise [0,0,1].
        savefile : str or None
            If provided, save the plot to this filename.
        """
        # Determine field direction
        if field_dir is None:
            field_dir = getattr(self, 'field_dir', np.array([0, 0, 1]))
        field_dir = np.array(field_dir) / np.linalg.norm(field_dir)

        self.Kappa = self.g0

        print(f'Using Kappa = {self.Kappa}')
        # Prepare path
        if path_labels is None:
            path_labels = ['Γ','X','W','K','Γ','L','U','W','L','K']

        # Build points from class high-symmetry vectors
        hs = {
            'Γ': self.FBZ_Gamma_GCC,
            'X': self.FBZ_X_GCC,
            'W': self.FBZ_W_GCC,
            'K': self.FBZ_K_GCC,
            'L': self.FBZ_L_GCC,
        ...
        for a, b in zip(path_labels[:-1], path_labels[1:]):
            ...
            for i in range(npoints):
                ...
                kpoints.append(k)
                if len(kpoints) > 1:
                    total_dist += np.linalg.norm(kpoints[-1] - kpoints[-2])
                kdist.append(total_dist)
        # add final endpoint
        kpoints.append(hs[path_labels[-1]])
        if len(kpoints) > 1:
            total_dist += np.linalg.norm(kpoints[-1] - kpoints[-2])
        kdist.append(total_dist)

        kpoints = np.array(kpoints)
        kdist = np.array(kdist[:len(kpoints)])

        # Compute dispersions
        bands = []
        for q in kpoints:
            # reuse compute_dispersion which uses self.Kappa, self.U, self.W
            bands.append(self.compute_dispersion(q))
        bands = np.array(bands)

        # Save data to text file if savefile provided
        if savefile:
            # Save as numpy text file with columns: kdist, bands
            data_file = savefile.replace('.png', '.txt').replace('.pdf', '.txt')
            np.savetxt(data_file, np.column_stack([kdist, bands]), 
                      header='kdist energy', comments='# ')
            print(f'Data saved to {data_file}')

        # Plot if matplotlib available
        fig, ax = plt.subplots(figsize=(8,5))
        # bands shape: (n_kpoints, ) -> scalar dispersion; if multi-band, handle
        if bands.ndim == 1:
            ax.plot(kdist, bands, '-b')
        else:
            for i in range(bands.shape[1]):
                ax.plot(kdist, bands[:, i], '-b')

        # xticks at symmetry point boundaries
        # compute cumulative indices for path segment boundaries
        seg_positions = [0.0]
        cum = 0
        for i in range(len(path_labels)-1):
            cum += npoints
            seg_positions.append(kdist[min(cum, len(kdist)-1)])

        ax.set_xticks(seg_positions)
        ax.set_xticklabels(path_labels)
        ax.set_xlabel('k-path')
        ax.set_ylabel('Energy')
        ax.set_title('Dispersion along path ' + '->'.join(path_labels) + ' (field [111])')
...
def generate_K_points_cubic(H_range_min, H_range_max, nH, K_range_min, K_range_max, nK, L_range_min, L_range_max, nL):
    """
    Generate a 3D grid where each point is a linear combination of H_vector, K_vector, and L_vector
    with coefficients spanning [-H_range, H_range], [-K_range, K_range], [-L_range, L_range].
...
def get_hhl(H_range_min, H_range_max, nH, K_range_min, K_range_max, nK):
    H_vector = np.array([1, 1, 0])
...
def sssf_mag(B, field_dir):
    ...
    int_grid = get_hhl(h_min, h_max, nH, k_min, k_max, nK)
    Jzz = 0.063
    Jpm = 0.1825
    ...
    ax.imshow(reshaped_data.T, extent=(-3, 3, -3, 3), origin='lower', cmap='viridis', aspect='auto')
...
def dssf_integrated_K(Jzz, Jpm, B, dir, nH, h_range, nK, k_range, nL, l_range, generate_func):
    # Generate grid of K points in the specified region
    h_min, h_max = h_range
    k_min, k_max = k_range
    l_min, l_max = l_range
    int_grid = generate_func(h_min, h_max, nH, k_min, k_max, nK, l_min, l_max, nL)
    ...
    # Save the plot
    plt.savefig(dir + '/DSSF_H_omega_3D_{}.pdf'.format(B))
    # plt.show()
    ...
    plt.savefig(dir + '/integrated_spectrum_{}.pdf'.format(B))

    # Save the integrated spectrum to a file
    np.savetxt(dir + '/integrated_spectrum_{}.txt'.format(B), np.column_stack((omega_vals, integrated_spectrum)), header='Energy (meV) Intensity (arb. units)', fmt='%.6f')
    # plt.show()
    
    return integrated_spectrum

def animate_integrated_spectrum(dir='DSSF_CZO', H_values=None, output_filename='integrated_spectrum_animation.gif'):
    """
    ...
    for h in H_values:
        ...
        if os.path.exists(filename):
            ...
            print(f"Warning: File {filename} not found, skipping H = {h} T")
    ...
    ax.set_ylabel(r'Intensity (arb. units)', fontsize=14)
    ax.grid(True, alpha=0.3)
    
    # Text annotation for field strength
    field_text = ax.text(0.02, 0.95, '', transform=ax.transAxes, 
                        fontsize=16, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    
    # Title
    ax.set_title(r'Integrated Dynamical Structure Factor vs Magnetic Field', fontsize=16)
    ...
    def animate(i):
        ...
        B = valid_H_values[i] * mu_B / g
        field_text.set_text(f'H = {valid_H_values[i]:.2f} T\nB = {B:.4f} meV')
        return line, field_text
    
    # Create animation
    anim = FuncAnimation(fig, animate, init_func=init, 
                        frames=len(valid_H_values), interval=200, 
...
def main_dispersion():
    ...
    ham = QuantumSpinIcePhoton(Jpm=Jpm, Jzz=Jzz, B=B, field_dir=field_dir, 
                            kappa=1, W=0, temperature=0.0, speed_of_light=speed_of_light)
    points = [
    ...
    print("plotting")
    plot_dispersion_along_point(ham, points, savefig=True, file_output_name="dispersion_photon_temp_{0:.2f}.pdf".format(temperature))
    print("plotting")
...
if __name__=="__main__":
    # Plot dispersion along symmetry points with [111] field
    print("\n" + "="*70)
    print("PLOTTING DISPERSION ALONG HIGH-SYMMETRY PATH")
    print("="*70)
    
    # Physical parameters for [111] field
    Jzz = 0.063
    Jpm = 0.01825
    field_dir = np.array([1, 1, 1])

    
    print("\nDispersion plot saved to 'dispersion_111_field.png'")
    print("="*70 + "\n")
    
    # sssf_under_111_field()
    dispersion = "Dispersion_111_field"
    dir = "DSSF_CZO"
    dir_X = "DSSF_CZO_X"
    ...
    for h in H:
        B = h * mu_B / g
        print(f"Calculating for H = {h} T (B = {B} meV)")
        ham = QuantumSpinIcePhoton(Jpm=Jpm, Jzz=Jzz, B=B, field_dir=field_dir, 
                                kappa=1, W=0, temperature=0.0, speed_of_light=speed_of_light)
        # Plot dispersion along default path
        dispersion_file = dispersion + '/dispersion_111_field_{}.png'.format(h)
        print("Saving dispersion plot to {}".format(dispersion_file))
        ham.plot_dispersion_along_path(
            path_labels=['Γ','X','W','K','Γ','L','U','W','L','K'],
            npoints=50,
            field_dir=field_dir,
            savefile=dispersion_file
        )
        dssf_integrated_K(Jzz, Jpm, B, dir, 5, (-0.1, 0.1), 5, (0.739, 0.839), 5, (-0.1, 0.1), generate_K_points_pengcheng_dai)
        dssf_integrated_K(Jzz, Jpm, B, dir_X, 5, (-0.1, 0.1), 5, (-0.1, 0.1), 5, (0.9, 1.1), generate_K_points_cubic)
        sssf_mag(B, field_dir)
    # Create animations for both directories
    print("\nCreating animation for DSSF_CZO...")
    animate_integrated_spectrum(dir=dir, H_values=H, output_filename='integrated_spectrum_CZO_animation.gif')
    
    print("\nCreating animation for DSSF_CZO_X...")
    animate_integrated_spectrum(dir=dir_X, H_values=H, output_filename='integrated_spectrum_CZO_X_animation.gif')

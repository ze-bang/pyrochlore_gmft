...
def QFI_at_K_T_zero(Jxx, Jyy, Jzz, X, flux, n_sites):
    ...
    tempzz, globalzz, tempxx, globalxx, energy = SSSF_q_omega_at_K(X, Jxx, Jyy, Jzz, 0, h110, flux, n_sites, 0)
    Szz = 4*tempzz
    ...
    return Szz, Szzglobal, Sxx, Sxxglobal, energy
...
def QFI_scan(Xs, Xstring, n):
    ...
    for i in range(n):
        ...
        Szz[i], Szzglobal[i], Sxx[i], Sxxglobal[i], energy[i] = QFI_at_K_T_zero(Jxx, Jyy, Jzz, Xs, flux, 25)

    plt.plot(Jpms, energy, label="Energy at K")
    plt.legend()
    plt.xlabel(r"$J_{\pm}/|J_{zz}|$")
    plt.ylabel("Energy at K")
    plt.savefig("Energy_at_K.pdf")
    plt.clf()

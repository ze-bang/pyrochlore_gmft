...
MFEpiold = np.zeros(n)
for i in range(n):
    A = pygen.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h110, flux=np.zeros(4))
    B = pygen.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h110, flux=np.array([np.pi, np.pi, 0, 0]))
    ...
    D = pygen.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h110, flux=np.array([np.pi, np.pi, np.pi, np.pi]))
    A.solvemeanfield()
    B.solvemeanfield()
    # C.solvemeanfield()
    D.solvemeanfield()
    ...
    # MFE0[i] = C.MFE()
    MFEpiold[i] = D.MFE()
    print(JP[i], MFEpiold[i], MFEpp00[i], MFE0old[i])

plt.plot(JP, MFE0old, label='0')
plt.plot(JP, MFEpp00, label=r'$\pi\pi 0 0$')
# plt.plot(JP, MFE0, label='pi Flux weird')
plt.plot(JP, MFEpiold, label=r'$\pi$')
plt.legend()
plt.show()

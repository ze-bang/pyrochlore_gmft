...
def graphdispersion(JP, kappa, rho, graphres, BZres):
    if JP >= 0:
        py0s = py0.zeroFluxSolver(JP, kappa=rho, graphres=graphres, BZres=BZres)
        py0s.setupALL()
        py0s.findLambda()
        print(py0s.lams)
        py0s.graph(0, False)
        ...
        py0s = pypi.piFluxSolver(JP, kappa=rho, graphres=graphres, BZres=BZres)
        py0s.setupALL()
        py0s.findLambda()
        py0s.graph(0, False)
        py0s.graph(1, True)

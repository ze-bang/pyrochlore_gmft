...
for i in range(n):
    ...
    B = pygen.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h111, flux=np.array([np.pi, np.pi, np.pi, np.pi]))
    # C = pygen.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h110, flux=np.zeros(4))
    D = pypi.piFluxSolver(-2*JP[i], -2*JP[i], 1, kappa=2, graphres=graphres, BZres=35, h=0.3, n=h111)
    # A.solvemeanfield()
    B.solvemeanfield()
    # C.solvemeanfield()
    D.solvemeanfield()

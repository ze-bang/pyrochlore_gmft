...
def DSSFgraphGen(h0, hmid, mid, hpi, n, filename):
    ...
    fig, axs = plt.subplots(nrows=2,ncols=3, figsize=(22, 14),layout="constrained")
    ...
    fig.supylabel(r'$\omega/J_{\parallel}$')
    ...
    d = np.loadtxt("../Data/Final_DSSF/Jpm=0.02/h"+dirString+"="+str(h0)+"_Szz_global.txt")
    ...
    d = np.loadtxt("../Data/Final_DSSF/Jpm=-0.03/h"+dirString+"="+str(hmid)+"_Szz_global.txt")
    emin, emax = np.min(emin) * 0.95, np.max(emax)*1.02
    ...
    d = np.loadtxt("../Data/Final_DSSF/Jpm=-0.289/h"+dirString+"="+str(hpi)+"_Szz_global.txt")
    emin, emax = np.min(emin) * 0.95, np.max(emax)*1.02
...
def SSSFgraphGen(h0, hmid, fluxmid, hpi, n, tograph, filename, colors=np.array(['w','w','w','w','w','w','w','w','w','w'])):
    ...
    fig, axs = plt.subplots(nrows=3,ncols=3, figsize=(16, 14), layout="tight")

    if len(hpi) == 2:
        axs[-1, -1].axis('off')

    axs[0,0].set_xlabel(r'$J_\pm/J_{\parallel}=0.02$', labelpad=10)
    axs[0,0].xaxis.set_label_position('top')
    axs[0,1].set_xlabel(r'$J_\pm/J_{\parallel}=-0.03$', labelpad=10)
    axs[0,1].xaxis.set_label_position('top')
    axs[0,2].set_xlabel(r'$J_\pm/J_{\parallel}=-0.3$', labelpad=10)
    axs[0,2].xaxis.set_label_position('top')


    fig.text(1, 0.85, r'$h/J_{\parallel}=' + str(h0[0]) + '$', ha='right', va='center', rotation=270)
    fig.text(1, 0.52, r'$h/J_{\parallel}=' + str(h0[1]) + '$', ha='right', va='center', rotation=270)
    fig.text(1, 0.2, r'$h/J_{\parallel}=' + str(h0[2]) + '$', ha='right', va='center', rotation=270)
...
# a.graph(axs)
# plt.show()
# DSSFgraphGen(0.2,0.2,np.array([0,0,0,0]),0.05,h001,"h001_DSSF.pdf")
# DSSFgraphGen(0.4,0.4,np.array([0,0,np.pi,np.pi]),0.1,h110,"h110_DSSF.pdf")
...
# SSSFGraphHHL(np.ones((4,4)), "test", 2, 2)

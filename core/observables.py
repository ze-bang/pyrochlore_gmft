...
import core.pyrochlore_gmft as pycon
...
# Lookup dictionaries for HKL coordinate conversions and graph functions
# These map hkl plane names to their respective conversion/graphing functions
HKL_TO_K_FUNCS = {
    "hk0": hkztoK,
    "hnhl": hnhltoK,
    "hhl": hhltoK,
    "hhknk": hhknktoK,
    "hh2k": hnhkkn2ktoK,  # default fallback
    "hkk": hhknktoK,  # hkk uses same as hhknk
}

HKL_GRAPH_FUNCS = {
    "hk0": None,  # Will be set after function definitions
    "hnhl": None,
    "hhl": None,
    "hhknk": None,
    "hh2k": None,
...
def hkl_to_K(hkl, A, B, K_offset=0):
    """
    Convert HKL plane coordinates to K-space vectors.
    
    Args:
        hkl: String specifying the plane type ("hk0", "hnhl", "hhl", "hhknk", "hkk", "hh2k")
        A, B: Meshgrid arrays of coordinates
        K_offset: Offset value for the third coordinate (default 0)
    
    Returns:
        K-space vectors reshaped to (nK*nK, 3)
    """
    nK = A.shape[0]
    convert_func = HKL_TO_K_FUNCS.get(hkl, hnhkkn2ktoK)  # default fallback
    return convert_func(A, B, K_offset).reshape((nK * nK, 3))


def get_hkl_graph_func(hkl):
    """
    Get the appropriate graph function for the given HKL plane type.
    
    Args:
        hkl: String specifying the plane type
    
    Returns:
        The corresponding SSSFGraph function
    """
    return HKL_GRAPH_FUNCS.get(hkl, HKL_GRAPH_FUNCS.get("hkk"))  # default to hkk


def SSSF_plot_from_file(folder, hkl, Hr=4, Lr=4, vmin=np.nan, vmax=np.nan, cmap='jet', 
                         components=None, interpolation='lanczos', output_suffix=''):
    """
    Plot SSSF data from previously saved text files without recalculation.
    
    Args:
        folder: Path to folder containing SSSF output files (e.g., 'SSSF_hhl')
        hkl: String specifying the plane type ("hk0", "hnhl", "hhl", "hhknk", "hkk", "hh2k")
        Hr: Horizontal range extent (default 4)
        Lr: Vertical range extent (default 4) 
        vmin: Minimum value for colormap (default np.nan for auto)
        vmax: Maximum value for colormap (default np.nan for auto)
        cmap: Colormap name (default 'viridis')
        components: List of components to plot. Options: 'Szz', 'Szzglobal', 'SzzNSF', 
                    'Sxx', 'Sxxglobal', 'SxxNSF'. Default None plots all available.
        interpolation: Interpolation method for imshow (default 'lanczos')
        output_suffix: Suffix to append to output filenames (default '')
    
    Example:
        SSSF_plot_from_file('SSSF_hhl', 'hhl', Hr=4, Lr=4, vmin=0, vmax=1, cmap='inferno')
    """
    # Default components if not specified
    if components is None:
        components = ['Szz', 'Szzglobal', 'SzzNSF', 'Sxx', 'Sxxglobal', 'SxxNSF']
    
    graph_func = get_hkl_graph_func(hkl)
    
    for comp in components:
        filepath = os.path.join(folder, comp + '.txt')
        if os.path.exists(filepath):
            data = np.loadtxt(filepath)
            # Use the custom plotting helper with additional options
            _SSSF_plot_helper(data, folder + '/' + comp + output_suffix, hkl, Hr, Lr, 
                             vmin, vmax, cmap, interpolation)
            print(f"Plotted {comp} from {filepath}")
        else:
            print(f"File not found: {filepath}")


def _SSSF_plot_helper(d1, filename, hkl, Hr, Lr, vmin, vmax, cmap, interpolation):
    """
    Helper function for plotting SSSF data with custom colormap settings.
    """
    # Plot the data
    if np.isnan(vmin) or np.isnan(vmax):
        plt.imshow(d1, interpolation=interpolation, origin='lower', 
                   extent=[-Hr, Hr, -Lr, Lr], aspect='auto', cmap=cmap)
    else:
        plt.imshow(d1, interpolation=interpolation, origin='lower', 
                   extent=[-Hr, Hr, -Lr, Lr], aspect='auto', cmap=cmap,
                   vmin=vmin, vmax=vmax)
    ...
    # Add HKL-specific BZ boundaries and labels
    if hkl == "hhl":
...
def quantum_fisher_information_K(D, temp):
    size = int(np.sqrt(D.shape[0]))
    omega = np.linspace(0, 20, D.shape[1])
    if not temp == 0:
        beta = 1/(temp)
...
def DSSF_core_pedantic(q, omega, pyp0, tol, beta=0):
    ...
    Szz = (np.real(Spm) + np.real(Spp))
    Sxx = (np.real(Spm) - np.real(Spp))
...
def SSSF_core_pedantic(q, v, pyp0):
    ...
    if pyp0.dominant == 0:
        Sxx = (np.real(Spm) + np.real(Spp)) / 2 / np.pi
        Szz = (np.real(Spm) - np.real(Spp)) / 2 / np.pi
    else:
        Szz = (np.real(Spm) + np.real(Spp)) / 2 / np.pi
        Sxx = (np.real(Spm) - np.real(Spp)) / 2 / np.pi
...
def plot_BZ(offset, boundary, color='w--'):
    """Plot Brillouin zone boundary as a closed polygon with dashed lines.
    
    Args:
        offset: 2D offset to apply to the boundary vertices
        boundary: Nx2 array of boundary vertices
        color: matplotlib color/linestyle string (default 'w--' for white dashed)
    """
    B = boundary + offset
    n_vertices = len(B)
    for i in range(n_vertices):
        plot_line(B[i], B[(i + 1) % n_vertices], color)
...
def SSSFGraphHK0(d1, filename, Hr, Lr, vmin=np.nan, vmax=np.nan):
    ...
    # High symmetry points for HK0 plane
    Gamms = np.array([[0,0],[2,0],[0,2],[-2,0],[0,-2],[2,2],[-2,2],[2,-2],[-2,-2]])
    Xs = np.array([[1, 0]])
    Ks = np.array([[1,1]])*0.375/0.5
    Ws = np.array([[1,0.5]])

    # BZ boundary for HK0 plane
    Boundary = np.array([[1, 0.5], [0.5,1], [-0.5,1], [-1,0.5],[-1,-0.5],[-0.5,-1],[0.5,-1],[1,-0.5]])

    # Plot BZ boundaries with dashed lines
    plot_BZ(Gamms[0], Boundary, 'w--')
...
def SSSFGraphHH2K(d1, filename, Hr, Lr, vmin=np.nan, vmax=np.nan):
    ...
    # BZ boundary for HH2K plane (similar to HHL but stretched)
    Gamms = np.array([[0,0]])
    Boundary = np.array([[0.25, 1],[-0.25,1],[-0.75,0],[-0.25,-1],[0.25,-1],[0.75,0]])
...
def SSSFGraphHnHL(d1, filename, Hr, Lr, vmin=np.nan, vmax=np.nan):
    ...
    # BZ boundary for HnHL plane (same as HHL plane)
    Gamms = np.array([[0,0]])
    Boundary = np.array([[0.25, 1],[-0.25,1],[-0.75,0],[-0.25,-1],[0.25,-1],[0.75,0]])
...
# Initialize the HKL graph functions lookup dictionary
# This must be done after the graph functions are defined
HKL_GRAPH_FUNCS.update({
    "hk0": SSSFGraphHK0,
    "hnhl": SSSFGraphHnHL,
    "hhl": SSSFGraphHHL,
    "hhknk": SSSFGraphHKK,
...
def SSSF_pedantic(nK, Jxx, Jyy, Jzz, h, n, flux, BZres, filename, hkl, *args, K=0, Hr=2.5, Lr=2.5, g=0, theta=0):
    ...
    if rank == 0:
        ...
        # Get the appropriate graph function for this hkl plane
        graph_func = get_hkl_graph_func(hkl)

        # Save data files (common to all hkl types)
        ...
        # Generate graphs using the appropriate function
        graph_func(Szz, filename+'/Szz', Hr, Lr)
        graph_func(Szzglobal, filename + '/Szzglobal', Hr, Lr)
        graph_func(Sxx, filename + '/Sxx', Hr, Lr)
        graph_func(Sxxglobal, filename + '/Sxxglobal', Hr, Lr)
        graph_func(d5, filename + '/SzzNSF', Hr, Lr)
        graph_func(d6, filename + '/SxxNSF', Hr, Lr)
        pedantic_SSSF_graph_helper(graph_func, d1, f1, Hr, Lr, n)
        pedantic_SSSF_graph_helper(graph_func, d2, f2, Hr, Lr, n)
        pedantic_SSSF_graph_helper(graph_func, d3, f3, Hr, Lr, n)
        pedantic_SSSF_graph_helper(graph_func, d4, f4, Hr, Lr, n)
...
def SSSF_q_omega_at_K(K, Jxx, Jyy, Jzz, h, n, flux, BZres, theta):
    py0s = pycon.piFluxSolver(Jxx, Jyy, Jzz, BZres=BZres, h=h, n=n, flux=flux, theta=theta)
    py0s.solvemeanfield()
    if not MPI.Is_initialized():
        MPI.Init()
    ...
    d1, d2, a, d3, d4, b = graph_SSSF_pedantic(py0s, K, n, rank, size)

    return contract('iab->i',d1), contract('iab->i',d2), contract('iab->i',d3), contract('iab->i',d4), py0s.MFE() - 0.5


def DSSF_q_omega_beta_at_K(beta, K, nE, Jxx, Jyy, Jzz, h, n, flux, BZres, theta):
    py0s = pycon.piFluxSolver(Jxx, Jyy, Jzz, BZres=BZres, h=h, n=n, flux=flux, theta=theta)
    py0s.solvemeanfield()
    ...
    return contract('ijab->ij',d1), contract('ijab->ij',d2), contract('ijab->ij',d3), contract('ijab->ij',d4)
...
def SSSF(nK, Jxx, Jyy, Jzz, h, n, flux, BZres, filename, hkl, K=0, Hr=2.5, Lr=2.5):
    ...
    if rank == 0:
        ...
        # Get the appropriate graph function for this hkl plane
        graph_func = get_hkl_graph_func(hkl)
        for d, f in [(d1, f1), (d2, f2), (d3, f3), (d4, f4), (d5, f5), (d6, f6), (d7, f7), (d8, f8)]:
            graph_func(d, f, Hr, Lr)
...
def DSSF_pedantic(nE, Jxx, Jyy, Jzz, h, n, flux, BZres, filename,theta=0,beta=0):
    ...
    d1, d2, d3, d4 = graph_DSSF_pedantic(py0s, e, kk, tol, rank, size, beta)
...
def DSSF_pedantic_custom_Ks(nE, kk, Jxx, Jyy, Jzz, h, n, flux, BZres, filename,theta=0,beta=0):
    pathlib.Path(filename).mkdir(parents=True, exist_ok=True)
    py0s = pycon.piFluxSolver(Jxx, Jyy, Jzz, BZres=BZres, h=h, n=n, flux=flux,theta=theta)
    py0s.solvemeanfield()
    lowedge, upedge = py0s.loweredge(), py0s.upperedge()
    ...
    if rank == 0:
        ...
        DSSFgraph(np.abs(Szz/np.max(Szz)), filename+"/Szz", kline, e)
        DSSFgraph(np.abs(Szzglobal/np.max(Szzglobal)), filename+"/Szzglobal", kline, e)
        DSSFgraph(np.abs(Sxx/np.max(Sxx)), filename+"/Sxx", kline, e)
        DSSFgraph(np.abs(Sxxglobal/np.max(Sxxglobal)), filename+"/Sxxglobal", kline, e)
